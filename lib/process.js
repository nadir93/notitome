...
const proc = {
  ...
  start: async(client, user) => {
    ...
    if (!util.getMySaveMoney()) {
      ...
      //TODO: in config 해당사용자 적립금 업데이트 
      const index = _.findIndex(config.users, o => {
        return o.id === user.id;
      });
      config.users[index].savedMoney = parseInt(mySaveMoney.replace(',', ''));
  ...
  showMeTheMoney: async client => {
    ...
    el = await client.element(util.getSelector('save_money'));
