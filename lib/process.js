...
function notifySavedMoney(user) {
  if (saveMoney > 0) {
    ...
    // 해당 사용자 적립금 업데이트
    const index = _.findIndex(config.users, o => o.id === user.id);
...
proc = {
  login: async (client, user) => {
    ...
    await client.waitForExist(util.getSelector('login_btn'), config.defaultWaitTime);
  ...
  start: async (client, user) => {
    ...
    if (!util.getMySaveMoney()) {
      ...
      const mySaveMoney = await client.getText(util.getSelector('infomation_my_save_money'));
    ...
    // const element = await client.getElementSize(util.getSelector('fullItemLayout'));
    const location = await client.getLocation(util.getSelector('fullItemLayout'));
    log.debug('| start location.length: ', location.length);
    const locationY = location.y ? location.y : location[location.length - 1].y;
  ...
  showMeTheMoney: async (client) => {
    ...
    log.debug('| el.status: ', el.status);
    if (el.status == null || el.status === 7) { // NoSuchElement
      log.debug('| [이미 적립됨]');
      return;
    }
  ...
  stop: async (client) => {
    await client.waitForExist(util.getSelector('fullItemLayout'), config.defaultWaitTime);
    const size = await client.getElementSize(util.getSelector('fullItemLayout'));
  ...
  sendMessage: (user) => {
    ...
    web.chat.postMessage(config.channel, null, {
      ...
      unfurl_links: true,
      as_user: false,
      icon_url: 'https://lh5.ggpht.com/o2TT2aEVw9kGTk0CryRAGpTZsotvo7ZmwDhXzq6bnXmPX4p15I0g6Roh6UB5VRx00uU=w300',
      username: '노티투미',
    }, (err, res) => {
      if (err) {
