...
module.exports = {
  process: async(client, data) => {
    ...
    await client
      .waitForVisible(util.getSelector('ad_image'), config.defaultWaitTime);
    await client.click(util.getSelector('ad_image'));
    await client.waitForVisible(
      util.getSelector('btn_ad_join'), config.defaultWaitTime);
    await client.click(util.getSelector('btn_ad_join'));
    await client.pause(400);
    await client.back();
    await client.waitForVisible(util.getSelector('btn_ad_save'), //com.interpark.notitome:id/popup_ib_close //reward_bg
      config.defaultWaitTime);
    await client.click(util.getSelector('btn_ad_save'))
    await client.pause(400)

    const response = await client.isExisting(
      util.getSelector('popup_ib_close'));
