/**
 * Author @nadir93
 * Date 2017.5.1
 */
'use strict';

const Logger = require('bunyan');
const loglevel = 'debug';
const log = new Logger.createLogger({
  name: 'messageType',
  level: loglevel
});

const util = require('./util');
const config = require('../config');

module.exports = {
  process: function(client, data) {
    return new Promise((resolve, reject) => {

      if (data.location.value.y > 320) {
        log.debug('| [이미 적립됨]');
        return resolve({});
      }

      log.debug('| 적립금: ', data.text.value + ' 원');
      client
        .waitForVisible(util.getSelector('ad_image'),
          config.defaultWaitTime)
        .click(util.getSelector('ad_image'))
        .waitForVisible(util.getSelector('btn_ad_join'),
          config.defaultWaitTime)
        .click(util.getSelector('btn_ad_join'))
        .pause(400)
        .back()
        .waitForVisible(util.getSelector('btn_ad_save'), //com.interpark.notitome:id/popup_ib_close //reward_bg
          config.defaultWaitTime)
        .click(util.getSelector('btn_ad_save'))
        .pause(400)
        .isExisting(util.getSelector('popup_ib_close'))
        .then(function(response) {
          log.debug('popup_ib_close exist: ', response);
          if (response) {
            data.text.value = 0;
            return client.back();
          }
          return client;
          //return response ? client.back() : client;
        })
        .back()
        .then(function() {
          resolve({
            saveMoney: data.text.value
          });
        })
        .catch(function(e) {
          log.error('error: ', e);
          //client
          //  .waitForVisible(util.getSelector('popup_ib_close'),
          //    config.defaultWaitTime)
          //적립금 받기 버튼이 없을 경우 그냥 skip하도록 처리해야함
          reject(e);
        });
    });
  }
};
